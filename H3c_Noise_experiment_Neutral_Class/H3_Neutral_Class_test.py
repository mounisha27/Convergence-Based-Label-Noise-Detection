"""
H3c: Does convergence-based detection work when noise targets the
NEUTRAL class (middle-sized, ~2,621 examples) — completing the H3
class-size sweep alongside H3a (negative, minority) and H3b (positive,
majority).

Identical experiment design to H1/H3b — only target_class changes.
Saved to separate output files so earlier results are not overwritten.
"""
...
def main():
    ...
    print("\nStep 3: Running H3c experiment (target_class = NEUTRAL)...")
    results = run_noise_balancing_experiment(
        X_text=df_labeled['reviews_text'],
        y_clean=df_labeled['sentiment'],
        target_class="neutral",   # <-- the only real change from H3b
        noise_rates=(0.0, 0.10, 0.25, 0.40),
    )
    results.to_csv("h3c_neutral_noise_results.csv", index=False)
    print("\nSaved to h3c_neutral_noise_results.csv")

    print("\nStep 4: Computing CV convergence score...")
    cv_results = compute_convergence_cv(results)
    cv_results.to_csv("h3c_neutral_convergence_cv.csv", index=False)
